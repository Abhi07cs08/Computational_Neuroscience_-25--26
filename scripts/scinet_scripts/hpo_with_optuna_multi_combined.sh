#!/bin/bash
#SBATCH --array 1-50%3
#SBATCH --job-name=hpo_optuna_simclr_combined
...
OTPUNA_STUDY_NAME=${2-"combined_spectral_loss_coeff_and_target_alpha"}

OPTUNA_DB=$SCRATCH/${OTPUNA_STUDY_NAME}.db

eval_args=${1-"--tune_spectral_loss_coeff --tune_target_alpha"}
more_args=${3-"--spectral_loss_coeff_min 0.5 --spectral_loss_coeff_max 3.0 --spectral_loss_coeff_step 0.03 --target_alpha_min 0.5 --target_alpha_max 3.0 --target_alpha_step 0.03"}

python -u "/home/kostouso/CompNeuro/Computational_Neuroscience_-25--26/scripts/hpo_train_multi.py" --optuna_db $OPTUNA_DB --optuna_study_name $OTPUNA_STUDY_NAME --imagenet_root /home/kostouso/CompNeuro/Computational_Neuroscience_-25--26/split_data --epochs 200 --batch_size 512 --save_dir "/scratch/kostouso/CompNeuro/Computational_Neuroscience_-25--26/Feb_7_launch/sk_logs_spec_loss_optuna_multi_test" --tau 0.2 --lr 0.1 --eval_every 0 --spectral_loss_warmup_epochs 10 --wd 1e-6 --workers 16 --lp_epochs 5 --lp_lr 0.1 --amp --seed 0  --neural_data_dir "/home/kostouso/CompNeuro/Computational_Neuroscience_-25--26/src/latest_neural_data/majajhong_cache" $eval_args $more_args
